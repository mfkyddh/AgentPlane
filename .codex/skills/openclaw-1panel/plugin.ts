...
function createClientFromPluginConfig(config: PluginConfig): OnePanelClient {
  return new OnePanelClient({
    ...
    securityEntrance:
      config.securityEntrance || process.env.ONEPANEL_SECURITY_ENTRANCE || undefined,
    ...
    api.registerTool(
      {
        ...
        description:
          'Interact with a 1Panel instance through signed API calls. Supports listing modules, listing actions, running a module action, or making a raw signed request.',
        parameters: {
          type: 'object',
          additionalProperties: false,
          properties: {
