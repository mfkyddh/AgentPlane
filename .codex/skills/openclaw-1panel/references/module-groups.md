...
When the panel enables a security entrance, requests also need:
...
- `ONEPANEL_BASE_URL`
- optional: `ONEPANEL_CONNECT_BASE_URL`
- `ONEPANEL_API_KEY`
...
- The 1Panel API interface must be enabled.
- If API IP whitelisting is enabled, the caller's real outbound IP must be allowed.
- In this repository, prefer SSH into the target host and call the API from that host. For `prod0-main`, the verified env path is `secrets/services/onepanel-api.env` locally and `/opt/env_ubuntu/secrets/services/onepanel-api.env` remotely.
- When `BindDomain` is enabled, use the bound public origin in `ONEPANEL_BASE_URL`, but point `ONEPANEL_CONNECT_BASE_URL` at the host-local listener such as `http://127.0.0.1:2096`.
...
- `POST /api/v2/websites/:id/https`

Verified writes currently cover create, SSL upload, and HTTPS binding. Destructive or broader mutations such as delete, operate, domain rewrites, and generic config rewrites remain reserved.
...
Use the installed app detail to resolve app status. Use the container module for app runtime logs. App uninstall, stop, restart, and other destructive operations remain reserved.

Repository-verified settings note:

- `POST /api/v2/core/settings/search` returns the current API config fields including `apiInterfaceStatus`, `apiKey`, `ipWhiteList`, `apiKeyValidityTime`, `bindDomain`, and `securityEntrance`.
- `POST /api/v2/core/settings/api/config/update` is currently blocked for API-key-authenticated callers on `prod0-main`.
- `POST /api/v2/core/settings/update` with `{"key":"IpWhiteList","value":"127.0.0.1"}` is the verified API-key path for tightening the whitelist.
- This whitelist only constrains direct access to the 1Panel listener. If a public reverse proxy on the same host forwards `/api/v2/...`, that reverse proxy may still appear as `127.0.0.1` to 1Panel.
