#!/usr/bin/env node
import { buildOnePanelHeaders, normalizeSecurityEntrance, OnePanelClient } from './client.js';
...
Usage:
  ...
  openclaw-1panel sign [--base-url URL] [--connect-base-url URL] [--api-key KEY] [--security-entrance ENTRANCE]
  openclaw-1panel request <METHOD> <PATH> [--body-json JSON] [--query-json JSON] [--node NODE] [--connect-base-url URL] [--security-entrance ENTRANCE]
  openclaw-1panel run <module> <action> [--input-json JSON] [--input-file FILE] [--node NODE] [--connect-base-url URL] [--security-entrance ENTRANCE]

Environment:
  ONEPANEL_BASE_URL
  ONEPANEL_CONNECT_BASE_URL
  ONEPANEL_API_KEY
  ONEPANEL_SECURITY_ENTRANCE
  ONEPANEL_TIMEOUT_MS
...
function handleSign(args) {
    ...
    const headers = buildOnePanelHeaders({
    ...
    console.log(JSON.stringify({
        ...
        connectBaseUrl: config.connectBaseUrl,
        securityEntrance: config.securityEntrance,
        headers,
    }, null, 2));
}
...
function resolveClientConfig(flags) {
    ...
    const connectBaseUrl = readOptionalString(flags['connect-base-url']) || process.env.ONEPANEL_CONNECT_BASE_URL;
    const apiKey = readOptionalString(flags['api-key']) || process.env.ONEPANEL_API_KEY;
    const securityEntrance = readOptionalString(flags['security-entrance']) ||
        process.env.ONEPANEL_SECURITY_ENTRANCE;
    if (!baseUrl) {
    ...
    return {
        ...
        connectBaseUrl,
        apiKey,
        securityEntrance: normalizeSecurityEntrance(securityEntrance),
    };
}
function createClient(flags) {
    ...
    return OnePanelClient.fromEnv({
        ...
        ONEPANEL_CONNECT_BASE_URL: config.connectBaseUrl,
        ONEPANEL_API_KEY: config.apiKey,
        ONEPANEL_SECURITY_ENTRANCE: config.securityEntrance,
    });
}
