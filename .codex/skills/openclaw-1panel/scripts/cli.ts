...
function resolveClientConfig(flags: Flags): {
  baseUrl: string;
  ...
  const connectBaseUrl =
    readOptionalString(flags['connect-base-url']) || process.env.ONEPANEL_CONNECT_BASE_URL;
  const apiKey = readOptionalString(flags['api-key']) || process.env.ONEPANEL_API_KEY;
  const securityEntrance =
    readOptionalString(flags['security-entrance']) ||
