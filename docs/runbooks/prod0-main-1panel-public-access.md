...
- 公网站点治理：必须先创建为 1Panel 入口对象，再由 OpenResty 承载
...
如需补充读取 provider/debug 事实，可只读执行 `agentplane onepanel --env prod0-main panel get`，但不要把它当默认预检主入口。

如需定点核对某个站点对象，用正式 `ingress` 对象面，而不是页面点击或 raw provider workflow：

```bash
env -C <repo-root> agentplane ingress get \
  --target prod0-main \
  --alias token \
  --repo-root <repo-root>
...
- 如证书对象、目录映射或网站 HTTPS 绑定发生漂移，在受控窗口内回到 1Panel 入口对象与证书对象处理，不要把历史外部脚本当正式实现
...
- `ingress verify --target prod0-main --alias token --repo-root <repo-root>` 返回站点对象与反代事实一致
- `8443` 对外响应正常
