...
- `infra remote bash prod0-main` 的 Windows 入口和实际远端执行都可用。
...
1. `infra audit prod0-main` 仍报 `sub2api` 的 config file 路径没有收口到目标目录语义。
...
agentplane infra network audit prod0-main --repo-root <repo-root>
agentplane infra remote bash prod0-main --dry-run --script-file agentplane/scripts/internal/remote/example.sh --repo-root <repo-root>
```
...
printf '%s\n' 'set -euo pipefail' 'hostname' 'docker ps --format "{{.Names}}"' \
  | agentplane infra remote bash prod0-main --repo-root <repo-root>
```

### 2. `sub2api` 应用面

```bash
agentplane app resource verify --target prod0-main --app sub2api --repo-root <repo-root>
agentplane projection runtime-env verify --target prod0-main --app sub2api --repo-root <repo-root>
...
- `onepanel_openresty` 继续是 `infra` 网络模式。
