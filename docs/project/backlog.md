...
| ID | 任务 | 状态 | 服务主线条件 | 说明 |
|----|------|------|--------------|------|
| B1 | 1Panel 最新适配与扩展功能规划 | 进行中 | 条件二、Beta M5 Provider 合同 | 已建立 [1Panel 更新适配与扩展规划](onepanel-adaptation-expansion-plan.md)：基于本地 `D:\Projects\00GITHUB\1Panel` 最新代码，梳理 provider 更新同步机制、优先接入对象和扩展路线；先规划，不直接扩大 raw `onepanel` 公开面。 |
