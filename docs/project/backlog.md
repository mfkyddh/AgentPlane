...
| ID | 任务 | 状态 | 服务主线条件 | 说明 |
|----|------|------|--------------|------|
| B1 | 1Panel 最新适配与扩展功能规划 | 进行中 | 条件二、Beta M5 Provider 合同 | 已建立 [1Panel 更新适配与扩展规划](onepanel-adaptation-expansion-plan.md)，并实现 M1 首个 `repo provider onepanel route-fingerprint` 门禁命令；继续补 route diff 影响矩阵与 object API contract fixture。 |
