...
- 已扩展 `service verify` 对容器和 compose live 证据的覆盖：当 inventory 声明 `project_name`、`compose_file` 或 `config_files` 时，校验 Docker inspect 中的 compose project/config file labels，覆盖普通 compose 与 1Panel compose 运行态漂移。
- 已扩展 `ingress verify` 对网站、域名、HTTPS、SSL 证书详情（status）、OpenResty 运行状态的覆盖。新增 `get_website_ssl` 和 `get_openresty_status` API helper。
- 已扩展 `infra automation verify` 对 cronjob 最近执行记录的覆盖。新增 `search_cronjob_records` API helper，检查最近一次执行状态和时间。
- 已扩展 `infra network firewall-audit` 对 1Panel firewall rule 的只读对账。新增 `audit_firewall_rules` 函数，检查防火墙激活状态、声明规则与实际规则的漂移。
...
- 已扩展 `app resource verify` 增加 live provider 证据：当 1Panel API 可达时，查询数据库（mysql/postgresql/redis）并与声明的 app resource 交叉验证。新增 `search_databases` API helper 和 `search_onepanel_databases` Gateway 方法。live_provider 检查为可选——provider 不可达时 verify 正常进行。
