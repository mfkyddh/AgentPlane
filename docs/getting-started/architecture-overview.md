...
| 层级 | 类比 | 回答的问题 |
|------|------|-----------|
| **Inventory（台账）** | 仓库的**货物清单** | "我们声明了要管理哪些东西？" |
| **Ledger（证据）** | 仓库的**盘点记录** | "我们最近一次盘点时，实际看到了什么？" |
| **Summary（摘要）** | 仓库的**月度报告** | "当前状况如何？需要关注什么？" |

**关键区别**：
- **Inventory 是"声明"**：来自 Git 配置，是"我们希望系统是什么样"
- **Ledger 是"证据"**：来自现场验证，是"系统实际是什么样"
- **Summary 是"结论"**：来自 Inventory + Ledger，是"人类需要知道什么"
...
目标环境的**正式非敏感台账**，回答："我们声明了要管理哪些对象？"

**本质**：Inventory 是**声明**，来自 Git 配置。它记录的是"我们希望系统有什么"，而不是"系统实际有什么"。

**类比**：就像仓库的货物清单——清单上写了"应该有 100 箱苹果"，但实际有没有、苹果是否新鲜，需要去盘点才知道。
...
围绕某类对象生成的**机器派生记录**，回答："最近一次验证时，实际看到了什么？"

**本质**：Ledger 是**证据**，来自 Live State（现场验证）。它记录的是"系统实际有什么"，而不是"我们希望系统有什么"。

**类比**：就像仓库的盘点记录——盘点员去现场数了数，发现"实际只有 95 箱苹果，其中 5 箱已经烂了"。这个记录就是证据。

### Inventory vs Ledger：核心区别

| 维度 | Inventory（台账） | Ledger（证据） |
|------|-------------------|----------------|
| **本质** | 声明 | 证据 |
| **来源** | Git 配置 | Live State（现场验证） |
| **回答的问题** | "我们声明了什么？" | "我们验证了什么？" |
| **更新频率** | 低频（配置变更时） | 高频（每次验证时） |
| **消费者** | 配置管理、部署规划 | 审计、对账、故障排查 |
| **优先级** | 中 | 高（Live State > Inventory） |

**为什么需要分开？**

1. **对账需求**：发现"声明"和"实际"的不一致
   - Inventory 说"应该有 redis"，但 Ledger 说"redis 没在运行" → 问题！

2. **来源分离**：Git 配置和现场状态是不同的来源
   - 如果合并，无法区分"这是配置定义的"还是"现场验证的"

3. **职责分离**：每层有独立的消费者和更新频率
   - Inventory 给配置管理用，Ledger 给审计对账用
...
| 概念 | 含义 | 类比 |
|------|------|------|
| **真源 (Source of Truth)** | 某一类事实的正式来源 | 法庭上的"原始证据" |
| **投影 (Projection)** | 从真源派生的只读视图 | 数据库的"视图" |
| **Inventory（台账）** | 来自 Git 的**声明**，记录"我们希望系统有什么" | 仓库的"货物清单" |
| **Ledger（证据）** | 来自现场的**验证**，记录"系统实际有什么" | 仓库的"盘点记录" |
| **App Summary** | 面向人类的**摘要**，记录"当前状况如何" | 仓库的"月度报告" |
| **Live State** | 通过现场命令/API 获取的当前真实状态 | 仓库的"实际库存" |
| **Task Entry** | 面向 Agent 的正式任务入口 | 仓库的"操作手册" |
| **Skill** | AI Agent 的意图入口，负责路由到 CLI | 仓库的"调度员" |

**核心洞察**：Inventory 和 Ledger 的分离不是为了"存储不同的数据"，而是为了"区分不同的来源和用途"。Inventory 是"声明"，Ledger 是"证据"，两者结合才能发现"声明"和"实际"的不一致。
