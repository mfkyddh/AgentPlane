...
**三层状态在 AgentPlane 中的实现**：

三层状态是概念模型。在 AgentPlane 的实现中，它们对应三层投影：

| 概念层 | 实现层 | 说明 |
|--------|--------|------|
| 期望状态 | Git 中的配置文件和 `contract.yaml` | 权威定义，可回滚 |
| 实际状态 | live state（现场实时查询） | 通过 `agentplane ... verify` 获取 |
| 观测状态 | inventory + ledger + app summary | 结构化投影，用于对账和审计 |

投影链的详细定义见 [控制面核心合同](../architecture/control-plane.md)。
