...
printf '%s\n' \
  'set -euo pipefail' \
  '1panel version' \
| uv run python -m ops.cli remote bash prod0-main
```
...
1. 先实测生产机版本  
   `printf '%s\n' 'set -euo pipefail' '1panel version' | uv run python -m ops.cli remote bash prod0-main`
