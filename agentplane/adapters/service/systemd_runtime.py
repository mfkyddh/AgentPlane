...
def verify_systemd_service(
    repo_root: Path, target: str, definition: ServiceDefinition, declared: dict[str, Any] | None = None
) -> dict[str, Any]:
    ...
    checks: dict[str, dict[str, object]] = {
        "active": {
            "ok": active["ok"] and str(active["stdout"]).strip() == "active",
            "actual": str(active["stdout"]).strip(),
        },
        "enabled": {
            "ok": enabled["ok"] and str(enabled["stdout"]).strip() in {"enabled", "static"},
            "actual": str(enabled["stdout"]).strip(),
        },
    }
...
def plan_systemd_operation(
    repo_root: Path, target: str, definition: ServiceDefinition, operation: str
