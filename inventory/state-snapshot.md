...
> ⚠️ 本文件包含时间敏感状态，由 `agentplane repo health-check` 或人工维护更新。
> 长期文档中的状态查看方法见 `docs/runbooks/current-state-and-validation.md`。
...
2. `prod0-readonly` 的 1Panel 只读验证链路当前不可用。现场报错显示远端缺少正式 AgentPlane 控制面代码。
...
1. 继续保持 `projection runtime-env plan` 默认脱敏。
2. 修复 `prod0-readonly` 远端 onepanel helper 的分发/查找路径。
3. 把 prod0 的 config file 收口规则写成明确合同。
